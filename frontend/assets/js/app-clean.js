...
function canWrite(module) {
    ...
    const matrix = {
        ...
        STOREKEEPER: ['movements', 'product-serials', 'deliveries', 'inventories', 'alerts'],
        MANAGER: ['movements', 'product-serials', 'deliveries', 'inventories', 'alerts', 'purchase-requests', 'purchase-orders', 'suppliers'],
    };
...
async function renderPurchaseOrders() {
    ...
    requestSelect?.addEventListener('change', async () => {
        ...
        try {
            // On recharge le detail (avec ses lignes) car la liste ne contient
            // pas les articles demandes.
            const detail = await apiRequest(`/purchase-requests/${requestId}`);
            const item = (detail?.data?.items ?? [])[0];

            const warehouseField = form.elements.namedItem('warehouse_id');
            if (warehouseField) {
                warehouseField.value = String(detail.data.warehouse_id);
            }

            if (item) {
                const productField = form.elements.namedItem('product_id');
                const quantityField = form.elements.namedItem('quantity_ordered');
                const costField = form.elements.namedItem('unit_cost');

                if (productField) {
                    productField.value = String(item.product_id);
                }
                if (quantityField) {
                    quantityField.value = String(item.quantity_requested);
                }
                if (costField && item.preferred_unit_cost !== null && item.preferred_unit_cost !== undefined) {
                    costField.value = String(item.preferred_unit_cost);
                }
            }
        } catch (error) {
            feedback.textContent = error.message;
